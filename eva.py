...
def make_target_model_as_copy(model: torch.nn.Module) -> torch.nn.Module:
    target_model = copy.deepcopy(model.q_func)
...
class EVA(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _evaluate_model_and_update_recurrent_states(
        ...
        if self.recurrent:
            ...
            batch_av = self.model(batch_xs, eva=(len(self.value_buffer.embeddings) == self.value_buffer.capacity))
        return batch_av
        ...
        for i in range(len(batch_obs)):
            ...
            if self.batch_last_obs[i] is not None:
                ...
                transition = {
                    ...
                    "embedding": self.batch_last_embed[0],
                ...
                self.backup_store_if_necessary(self.batch_last_embed, self.t)
                if batch_reset[i] or batch_done[i]:
                    self.batch_last_obs[i] = None
                    self.batch_last_action[i] = None
                    self.batch_last_embed[i] = None
    ...
    def _trajectory_centric_planning(self, trajectories):
        ...
        for trajectory in trajectories:
            ...
            batch = torch.empty((self.len_trajectory, 4, 84, 84), device=self.device, dtype=torch.float32)
        ...
        return torch.tensor(np.concatenate(q_value, axis=0).astype(np.float32), device=self.device), \
               torch.tensor(embeddings, device=self.device)
